...
class simulation():
    ...
    def killPerson(self, i):
        np.delete( self.sigma, i, 0 )
        np.delete( self.sigma, i, 1 )
        N -= 1
...
s = simulation( np.arange(0, 15, 0.1), mu )
s.runSimulation( nsteps=100*30 )

from random import choice
alphas = np.copy( s.alpha )
for i in range(50):
    s.addPerson(choice(alphas))

s.runSimulation( nsteps=100*30 )

rows = []
rows.append(["t", "degree","freq"])
for t, dist in s.degreeDistLog:
    for degree,freq in dist.items():
        rows.append([t,degree,freq])
renderCSV(rows, "degreeFreq200WithNewcomers.csv")

"""
s.exportGraphCSV( "longTimeLargeNetwork" )

#howLongHaveYouKnownFriends?
rows = []
rows.append(["friendshipLength","t"])

ametb = {}
for t, sigma in s.sigmaLog:
    iN, jN = np.shape(sigma)
    for i in range(iN):
        for j in range(jN):
            if sigma[i,j] > 0:
                if (i,j) not in ametb:
    ...
    lengthOfRelationships = [ t - x for ij,x in ametb.items() ]
    rows += [ [x, t] for x in lengthOfRelationships ]
    
with open("longTimeLargeNetwork.friendshipLengthAll.csv", 'wb') as csvf:
    from csv import writer
    cw = writer(csvf)
