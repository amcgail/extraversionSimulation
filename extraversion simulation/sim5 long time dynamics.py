    ...
    for i in range(iN):
        for j in range(jN):
            if sigma[i,j] > 0:
                ...
                if (i,j) in ametb:
                    friendShipEndTimes.append( [t, t - ametb[(i,j)]] )
                    del ametb[ (i,j) ]
                    
    #lengthOfRelationships = [ t - x for ij,x in ametb.items() ]
    
rows = []
rows.append(["endTime", "fLength"])
rows += friendShipEndTimes

with open("longTimeLargeNetwork.justEndTimes.csv", 'wb') as csvf:
    from csv import writer
    cw = writer(csvf)
    for r in rows:
        cw.writerow(r)
