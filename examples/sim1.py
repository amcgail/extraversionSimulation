...
s.runSimulation( nsteps=100 )

from csv import writer
with open("simOutput.graph.csv", 'wb') as csvf:
    w = writer( csvf )

    w.writerow( ["StartTime", "EndTime", "Source", "Target", "weight"] )
        ...
        for j in range(s.N):
            ...
            for change in s.sigmaLog:
                ...
                if t_last is not None:
                    w.writerow( [t_last, change[0]] + r_last )
                
                t_last = change[0]
                r_last = [i, j, change[1][i,j]]

unhappiness = []
for change in s.sigmaLog:
    unhappiness.append( ( 
        change[0], 
        sum([ s.mu( 1 - s.alpha[i] / sum( change[1][i] ) ) if sum( change[1][i] ) > 0 else s.mu(1e10) for i in range(s.N) ])
    ) )
for u in unhappiness:
    print "%s,%s" % u